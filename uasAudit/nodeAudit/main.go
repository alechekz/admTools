...
func ExecAudit(config *ssh.ClientConfig, host string, detailed, summary *bytes.Buffer) {
	...
	// #1: monitors nodes manual restarts by select entries
	// which record contains 'restart' from "COMMAND LOG"
	hc.MonRestarts(client, detailed)
...
func main() {
	...
	// where the porgram's ouput shoud be saved.:wq

	var filename string
	flag.StringVar(&filename, "f", "", "name of file where Node Audit should be logged")
	...
	fmt.Fprintln(detailed, "Node Audit Started:")
	...
	var subject string
	if isPassed {
		subject = "Daily Nodes Restarts Audit [PASSED]"
		fmt.Fprintln(detailed, "\n\n\t/// AUDIT IS PASSED ///\n\n")
	} else {
		subject = "Daily Nodes Restarts Audit [FAILED]"
		fmt.Fprintln(detailed, "\n\n\t/// AUDIT IS FAILED ///\n\n")
	}
	...
	fmt.Fprintln(w, "\t/// NODE AUDIT SUMMARY INFORMATION ///")
	fmt.Fprintln(w, summary)
	fmt.Fprintln(w, "\n\t/// NODE AUDIT DETAILED INFORMATION ///\n")
	...
	if toMail {
		netHelper.EmailIt(filename, "Node-Audit", subject, "cc")
	}
